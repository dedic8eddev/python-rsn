...
#echo "$VIRTUAL_ENV_PATH/bin/gunicorn ${DJANGO_WSGI_MODULE}:application --name $NAME --workers $NUM_WORKERS --user=$USER --group=$GROUP --bind unix:$SOCKFILE --log-level=debug --log-file=$LOG_FILE --timeout 360"
exec gunicorn ${DJANGO_WSGI_MODULE}:application --name $NAME --workers $NUM_WORKERS   --timeout 360
