...
class Command(BaseCommand):
    ...
    def import_district(self):
        ...
        for item in tqdm(data, disable=self.options.get('quiet'), total=total, desc="Importing districts"):
            ...
            try:
                ...
                self.logger.debug("District: %d %s: Cannot find city in hierarchy, using nearest", geonameid,
                                  defaults['name'])
                city_pop_min = 100000
    ...
    def import_alt_name(self):
        ...
        for type_ in (Country, Region, Subregion, City, District):
            plural_type_name = '{}s'.format(type_.__name__) if type_.__name__[-1] != 'y' else \
                '{}ies'.format(type_.__name__[:-1])
        ...
        for item in tqdm(data, disable=self.options.get('quiet'), total=total,
