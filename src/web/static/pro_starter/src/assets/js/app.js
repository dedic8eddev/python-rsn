...
+function($, window){
  ...
  app.dir = {
    home:   app.corejs.replace('pro_assets/js/core.min.js', ''),
  ...
  //
  var assets_dir_el = $('[data-pro_assets-url]');
  if ( assets_dir_el.length ) {
    var assets_dir = assets_dir_el.data('pro_assets-url');
  ...
  app.isReady = function() {
    ...
    $(function(){
      ...
      var customPreloading = $('.custom-preloading');
      if ( preloader.length && customPreloading.length === 0) {
        var speed = preloader.dataAttr('hide-speed', 600);
  ...
  app.config = function(options) {
    ...
    if ( $('[data-provide~="map"]').length && window["google.maps.Map"] === undefined ) {
      if ( app.defaults.googleApiKey != null ) {
        $.getScript("https://maps.googleapis.com/maps/api/js?key="+ app.defaults.googleApiKey +"&libraries=places&callback=initMap&language=en");
      }
    }
