function handleActionFooter(){
    if ($('[name="ids"]:checked').length > 0) {
        const text = $('[name="ids"]:checked').length === 1 ? `${$('[name="ids"]:checked').length} entry` : `${$('[name="ids"]:checked').length} entries`;
        $(".action_item_number").html(text);
        $(".action-confirm").hide();
        $("#action").show();
        $(".action-main").show();
    } else {
...
function statusConfirmation(url, entities, newStatus){
    $("#action_main").hide();
    $("#action_delete_confirm").hide();
    $("#action_change_status_confirm").show();
    $(".action_new_status").html(newStatus);
    $("#ui-bootbox-change-status-confirm").click(function(){
