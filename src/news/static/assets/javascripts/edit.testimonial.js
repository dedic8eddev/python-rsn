...
var TransEngine = {
  ...
  get_current_contents: function() {
      var contents = $("#id_testimonial_textarea").val();
      return contents;
  },
  ...
  show_lang_version: function() {
      ...
      $("#id_testimonial_textarea").val(dd_tr_json['translations'][lang_ver]['text']);
  ...
  clear_dd_translations: function() {
      ...
      $.ajax({
          ...
          url: url_clear_dd,
          data: [
              {"name": "testimonial_id", "value": testimonial_id},
          ],
...
$(document).ready(function(){
    ...
    $('#id_connected_venue').select2({
        ...
        ajax: {
            ...
            url: "/ajax/autocomplete/place/for-featured-venue?for_testimonial=true&exclude_id="+venue_id,
            dataType: 'json',
            delay: 250,
    ...
    $('#id_selected_venue').select2({
        allowClear: true,
        width: '100%',
        placeholder: 'Select a venue and its Testimonial',
        dropdownParent: $('#selected_venue_selector'),
        ajax: {
            url: "/ajax/testimonials/testimonials-autocomplete",
            dataType: 'json',
            delay: 250,
            processResults: function (data, params) {
                for (var i in data.items){
    ...
    function getTestimonialList(data){
        var testimonialList = "";
        for (i = 0; i < data.length; i++){
                testimonialList += '<div class="task" data-title="'+ data[i].id+'" id="task'+ data[i].id+'">' +
                                '<div class="fa fa-arrows-v task-sort-icon"></div>' +
                                '<div class="action-checkbox">'+
                                    '<input type="checkbox" class="featred_testimonial_item" id="todo'+ data[i].id+'" name="featred_testimonial_item" value="'+ data[i].id+'">'+
                                    '<label for="todo'+ data[i].id+'" class="task-title">'+ data[i].text+  '</label>'+
                                '</div>'+
                            '</div>';
        }
        console.log(testimonialList)
        $("#sortable-list").html(testimonialList);
    }
    $('#btnDelFeaturedTestimonial').click(function(){
        if (confirm("Do you really want to delete selected testimonial?")) {
            $(".loader").css("display","flex");
            var selected = new Array();
            $('.featred_testimonial_item:checked').each(function() {
                selected.push(this.value);
            });
            $.ajax({
                type: "POST",
                url: "/testimonial/remove-featured-testimonials/",
                data: {  ids: JSON.stringify(selected) },
                dataType: 'json',
                success: function(data){
                    $(".loader").css("display","none");
                    getTestimonialList(data.data)
                },
                dataType: "json"
            });
        }
    ...
    var fnSubmit = function(){
        ...
        $.ajax({
            type: "POST",
            url: "/testimonial/change-featured-testimonial-order/",
    ...
    $('#btnAdd').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: "/testimonial/add-featured/"+$("#id_selected_venue").val()+'/',
            data: {},
            success: function(response){
                $(".loader").css("display","none");
                getTestimonialList(response.data);
