...
var TransEngine = {
  ...
  get_current_contents: function() {
      var contents = $("#id_cheffe_textarea").val();
      return contents;
  },
  ...
  show_lang_version: function() {
      ...
      $("#id_cheffe_textarea").val(dd_tr_json['translations'][lang_ver]['text']);
  ...
  clear_dd_translations: function() {
      ...
      $.ajax({
          ...
          url: url_clear_dd,
          data: [
              {"name": "cheffe_id", "value": cheffe_id},
          ],
...
$(document).ready(function(){
    ...
    $('#id_connected_venue').select2({
        ...
        ajax: {
            ...
            url: "/ajax/autocomplete/place/for-featured-venue?for_cheffe=true&exclude_id="+venue_id,
            dataType: 'json',
            delay: 250,
    ...
    $('#id_selected_venue').select2({
        allowClear: true,
        width: '100%',
        placeholder: 'Select a venue and its Cheffe',
        dropdownParent: $('#selected_venue_selector'),
        ajax: {
            url: "/ajax/autocomplete/place/for-featured-venue?for_cheffe_8_list=true",
            dataType: 'json',
            delay: 250,
            processResults: function (data, params) {
                for (var i in data.items){
    ...
    function getCheffeList(data){
        var cheffeList = "";
        for (i = 0; i < data.length; i++){
                cheffeList += '<div class="task" data-title="'+ data[i].id+'" id="task'+ data[i].id+'">' +
                                '<div class="fa fa-arrows-v task-sort-icon"></div>' +
                                '<div class="action-checkbox">'+
                                    '<input type="checkbox" class="featred_cheffe_item" id="todo'+ data[i].id+'" name="featred_cheffe_item" value="'+ data[i].id+'">'+
                                    '<label for="todo'+ data[i].id+'" class="task-title">'+ data[i].text+  '</label>'+
                                '</div>'+
                            '</div>';
        }
        $("#sortable-list").html(cheffeList);
    }
    $('#btnDelFeaturedCheffe').click(function(){
        if (confirm("Do you really want to delete selected cheffe?")) {
            $(".loader").css("display","flex");
            var selected = new Array();
            $('.featred_cheffe_item:checked').each(function() {
                selected.push(this.value);
            });
            $.ajax({
                type: "POST",
                url: "/cheffe/remove-featured-cheffes/",
                data: {  ids: JSON.stringify(selected) },
                dataType: 'json',
                success: function(data){
                    $(".loader").css("display","none");
                    getCheffeList(data.data)
                },
                dataType: "json"
            });
        }
    ...
    var fnSubmit = function(){
        ...
        $.ajax({
            type: "POST",
            url: "/cheffe/change-featured-cheffe-order/",
    ...
    $('#btnAdd').click(function(){
        ...
        $.ajax({
            type: "POST",
            url: "/cheffe/add-featured/"+$("#id_selected_venue").val()+'/',
            data: {},
            success: function(response){
                $(".loader").css("display","none");
                getCheffeList(response.data);
